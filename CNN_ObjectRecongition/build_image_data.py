...
tf.app.flags.DEFINE_string('train_directory', 'data/image', 'Training data directory')
tf.app.flags.DEFINE_string('validation_directory', './', 'Validation data directory')
tf.app.flags.DEFINE_string('output_directory', 'data/image', 'Output data directory')
...
tf.app.flags.DEFINE_string('labels_file', 'data/image/label.txt', 'Labels file')
...
def _convert_to_example(filename, image_buffer, label, text, height, width):
  ...
  example = tf.train.Example(features=tf.train.Features(feature={
      ...
      'image/filename': _bytes_feature(tf.compat.as_bytes(os.path.abspath(filename))),
...
def _process_image_files_batch(coder, thread_index, ranges, name, filenames, texts, labels, num_shards):
  ...
  for s in range(num_shards_per_batch):
    ...
    output_filename = '%s-%.5d-of-%.5d.tfdata' % (name, shard, num_shards)
